...
public class TicTacToeKataRunner implements CommandLineRunner {
		...
		try (Scanner inputReader = new Scanner(System.in)) {		
			do {				
				...
				if (Math.random() < 0.5) {
					...
					player1.setShape(GAME_STATE.CIRCLE);
					player2.setShape(GAME_STATE.CROSS);
				} else {
					System.out.println("X assigned to player 1 & O assigned to player 2");
					player1.setShape(GAME_STATE.CROSS);
					player2.setShape(GAME_STATE.CIRCLE);
				}
			...
			if (player1.getShape() == GAME_STATE.CROSS) {
			...
			if (ticTacToeGame.isFinished()) {
				GAME_STATE winner = ticTacToeGame.getWinner();
	...
	private void printGameBoardState() {
		GAME_STATE[][] board = ticTacToeGame.getBoard();
		
		for (int row = 0; row < board.length; row++) {
			for (int col = 0; col < board[row].length; col++) {
				if (GAME_STATE.CIRCLE == board[row][col]) {
					System.out.print(" O ");
				} else if (GAME_STATE.CROSS == board[row][col]) {
					System.out.print(" X ");
				} else {
					System.out.print(" - ");
				}				
